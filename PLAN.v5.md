<!-- v5 - 2026-05-07 - Generated via /improving-plans from PLAN.v4.md -->

# Yamaha RX-V583 Go CLI — Build Plan v5
...
Two design decisions resolved during the v4 design review, plus three small clarifying fixes. v5 is intentionally a small delta — the plan was already implementable; v5 closes two reliability gaps.

1. **DHCP resilience via UDN.** Each device entry persists its UPnP UDN (free from SSDP). On exit-69 from a config-resolved host, the CLI transparently re-runs SSDP filtered by UDN, updates the saved IP atomically, and retries. Stale-IP failure mode disappears.
2. **SIGINT cancellation via root context.** Cobra's root command builds its `context.Context` from `signal.NotifyContext`. All HTTP requests, the power-on poll, and `yamaha watch` honor it. `power on` cancelled mid-poll exits 130. `watch` SIGINT exits cleanly with code 0 (already in v4, now traced to a single mechanism).

Plus three micro-fixes:
- **`pkg/yxc.Client` documented as safe for concurrent use** (matches `net/http.Client` convention).
- **TTY table mode honors `NO_COLOR`** — universal convention; no flag needed.
- **Exit code 130** added to the table for SIGINT cancellation.
...
Yamaha receivers on home networks routinely get a new IP after DHCP renewals or router reboots. The CLI handles this transparently when the active device was resolved from config (i.e., not from `--host`).
...
**Skipped when:**
- Active device was resolved via `--host <ip>` (anonymous, no UDN to match).
...
3. `internal/config`: load multi-device YAML with the resolution order above; **atomic write** (`<file>.tmp` + `rename`); persist `udn` per device. Bind to viper. Global flags: `--host`, `--device`, `--zone`, `--output`, `--no-color`, `--debug`, `--no-wait`, `--refresh-features`.
...
8. **DHCP resilience** in `internal/cli`: when a config-resolved host fails after the in-client retry, call `discover.LookupByUDN`. On success, atomic-update the config and retry the command once. On failure, exit 69. Skipped for `--host` calls. Skipped for pre-v5 configs without UDN (exit 69 with hint).
9. `pkg/yxc.Client`:
...
- **DHCP shuffle:** manually edit config IP to a wrong-but-routable address (e.g., `192.168.1.250`); `yamaha status` succeeds after one transparent rediscover, and the config is updated to the real IP. Visible in `--debug` as `→ rediscover` then `→ getStatus` against the new IP.
